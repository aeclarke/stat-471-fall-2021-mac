...
```{r}
# try increasing noise level: increase sigma
bias_variance_tradeoff(f, sigma = 50, n, resamples)
```


```{r}
# try decreasing noise level
bias_variance_tradeoff(f, sigma = .5, n, resamples)
```
...
bias_variance_tradeoff(f, sigma, n = 25, resamples)
```

```{r}
# try increasing sample size
bias_variance_tradeoff(f, sigma, n = 100, resamples)
```
...
```{r}
bias_variance_tradeoff(f = function(x)(sin(10*x)), sigma, n, resamples)
```

```{r}
# try increasing complexity of f
bias_variance_tradeoff(f = function(x)(sin(2*x)), sigma = 5, n, resamples)
```
